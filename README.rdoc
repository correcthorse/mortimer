...
There is now a vagrant box to test out this software. It is provisioned via Ansible so you will need to be on an Ansible supported system to run it. It sets up mortimer from scratch on a CentOS 7 box using the built-in ruby 2.0. The bundle is installed and the prod/dev/test databases are created and setup. You can begin from the INSTALL section above.

After the installation steps are complete, you should be able to access https://localhost:8443 and log in.

If you'd like to access mysql as root, sudo to the root user and just run mysql, the credentials are saved in ~/.my.cnf

Make sure you run "vagrant up" from a clean clone or you may find some of your configuration files have been overwritten.
